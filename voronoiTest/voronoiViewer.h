...
{
	class voronoiViewer : public QMainWindow
	{
		Q_OBJECT
	public:
		voronoiViewer(const std::vector<geometry::Circle>& circles, const std::vector<geometry::Line>& lines, const std::vector<geometry::SemiCircle>& semicircles);
		~voronoiViewer();
		bool eventFilter(QObject* object, QEvent *event);
	protected:
		virtual bool event(QEvent* event);
	private:
		void addVoronoiLines();
